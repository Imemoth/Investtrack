...
function HistoricChart({ ticker, currency }) {
  ...
  useEffect(() => {
    ...
    const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(ticker)}?range=${range}&interval=${range === "1mo" ? "1d" : range === "3mo" ? "1d" : "1wk"}`;
    ...
    fetch(proxyUrl, { signal: AbortSignal.timeout(10000) })
      .then(r => r.text()).then(text => {
        ...
        const points = times.map((t, i) => ({ t: new Date(t * 1000), v: closes[i] })).filter(p => p.v != null);
        setData(points);
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {["1mo", "3mo", "1y", "5y"].map(r => (
            <button key={r} onClick={() => setRange(r)}
              style={{ background: range === r ? "#21262D" : "none", border: "1px solid " + (range === r ? "#30363D" : "transparent"), borderRadius: 6, padding: "3px 8px", color: range === r ? "#E6EDF3" : "#8B949E", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>
              {r}
            </button>
          ))}
        </div>
...
function NewsFeed({ ticker }) {
  ...
  useEffect(() => {
    if (!ticker) return;
    setLoading(true);
    const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(ticker)}?range=1d&interval=1d&includePrePost=false`;
    const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(yahooUrl)}`;
    // Yahoo news a summary-ban van — külön endpoint
    ...
    const newsProxy = `https://api.allorigins.win/raw?url=${encodeURIComponent(newsUrl)}`;
    fetch(newsProxy, { signal: AbortSignal.timeout(8000) })
      .then(r => r.text()).then(text => {
        const json = JSON.parse(text);
        setNews(json?.news || []);
      }).catch(() => setNews([]))
...
// ─── DETAIL MODAL ─────────────────────────────────────────────────────────────
function DetailModal({ inv, onClose, onEdit }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", backdropFilter: "blur(4px)", zIndex: 50, display: "flex", alignItems: "flex-end" }}
      ...
      <div style={{ background: "#161B22", border: "1px solid #30363D", borderRadius: "16px 16px 0 0", width: "100%", maxHeight: "90vh", overflow: "auto", boxShadow: "0 -8px 40px rgba(0,0,0,.6)" }}>
        ...
        <div style={{ padding: "20px 20px 0", borderBottom: "1px solid #21262D" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: CATEGORY_COLORS[inv.category] + "20", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 13, fontWeight: 800, color: CATEGORY_COLORS[inv.category], fontFamily: "'DM Mono', monospace" }}>{(inv.ticker || inv.name).slice(0, 4).toUpperCase()}</span>
              </div>
              <div>
          ...
          {/* Tabs */}
          <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #21262D", marginBottom: -1 }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                style={{ background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #6EE7B7" : "2px solid transparent", padding: "8px 14px", color: tab === t.id ? "#6EE7B7" : "#8B949E", cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit" }}>
                {t.label}
              </button>
            ))}
          </div>
...
function InvestmentForm({ initial, onSave, onCancel }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={grid2}>
        <div><label style={labelStyle}>🎯 Célár</label><input style={inputStyle} type="number" value={form.targetPrice || ""} onChange={e => set("targetPrice", e.target.value)} placeholder="Opcionális" /></div>
        <div><label style={labelStyle}>💰 Osztalék % / év</label><input style={inputStyle} type="number" value={form.dividendYield || ""} onChange={e => set("dividendYield", e.target.value)} placeholder="pl. 2.5" /></div>
      </div>
...
export default function App() {
  ...
  const [detailInv, setDetailInv] = useState(null); // "category" | "position"
    ...
    try {
      ...
      setInvestments(prev => {
        ...
        updated.forEach(inv => {
          ...
          const prev_inv = prev.find(p => p.id === inv.id);
          const wasBelow = prev_inv?.currentPrice < target;
          const nowAbove = inv.currentPrice >= target;
          if (wasBelow && nowAbove) {
            showToast(`🎯 ${inv.name} elérte a célárat! (${fmtNum(inv.currentPrice, 0)} ≥ ${fmtNum(target, 0)})`, "success");
          }
        });
        return updated;
      });
